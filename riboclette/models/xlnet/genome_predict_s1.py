...
df_set = pd.read_csv("pseudolabeling/data_preds/df_set1.csv")
...
for j in tqdm(range(len(sequences_df))):
    ...
    # prepend condition token
    X = [condition_values[conditions_df[j]]] + X
    ...
    df_full_sample = df_full[df_full['condition'] == conditions_df[j]]
    df_full_sample = df_full_sample[df_full_sample['transcript'] == transcripts_df[j]]
    ...
    if len(df_full_sample) > 0:
        ...
        final_annots_list.append(annots_sample)


    else:
        final_annots_list.append('NA')

    final_mean_preds_list.append(mean_preds)
    final_stds_preds_list.append(stds_preds)
    final_conditions_list.append(conditions_df[j])
    final_genes_list.append(genes_df[j])
    final_transcripts_list.append(transcripts_df[j])
    final_sequence_list.append(sequences_df[j])

# create a dataframe with the final predictions
df_final_preds = pd.DataFrame({'gene': final_genes_list, 'transcript': final_transcripts_list, 'sequence': final_sequence_list, 'mean_preds': final_mean_preds_list, 'stds_preds': final_stds_preds_list, 'condition': final_conditions_list, 'annotations': final_annots_list})
print(df_final_preds)
# save the dataframe
df_final_preds.to_pickle("pseudolabeling/data_preds/set1_preds.pkl")