...
def pseudolabelExp1(ground_truth, mean_preds):
...
def pseudolabelExp2(mean_preds):
...
def RiboDatasetExp1(threshold: float = 0.3, longZerosThresh: int = 20, percNansThresh: float = 0.05):
    ...
    exp1_preds = exp1_preds[~exp1_preds['gene'].isin(orig_val_genes)]
    exp1_preds = exp1_preds[~exp1_preds['transcript'].isin(orig_val_transcripts)]
    ...
    for i in tqdm(range(len(exp1_preds))):
        ...
        pred_sample = pseudolabelExp1(ground_truth_sample, mean_preds_sample)
    ...
    exp1_preds.to_csv(out_train_path)
...
def RiboDatasetExp2(threshold: float = 0.3, longZerosThresh: int = 20, percNansThresh: float = 0.05):
    ...
    # remove those for val genes and transcripts
    exp2_preds = exp2_preds[~exp2_preds['gene'].isin(orig_val_genes)]
    exp2_preds = exp2_preds[~exp2_preds['transcript'].isin(orig_val_transcripts)]
    ...
    for i in tqdm(range(len(exp2_preds))):
        ...
        pred_sample = pseudolabelExp2(mean_preds_sample)
    ...
    exp2_train = pd.concat([exp1_train, exp2_preds])

    exp2_train.to_csv(out_train_path, index=False)

    exp2_train = pd.read_csv(out_train_path)

    return exp2_train, df_val_orig, df_test_orig
